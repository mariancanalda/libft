...
/*   Updated: 2023/02/26 20:33:25 by macanald         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	*ft_calloc(size_t count, size_t size)
{
	...
	return (ptr);
			...
			// 2) 'total_size = count * size;' calcula el tamaño total 
			// de la memoria que se debe asignar para la matriz requerida.
			// La función 'ft_calloc' se utiliza para asignar un bloque de 
			// memoria de tamaño 'count * size' y devuelve un puntero al 
			// primer byte de la memoria asignada. Por lo tanto, 
			// 'count' es el número de elementos que se quieren almacenar 
			// en la memoria y 'size' es el tamaño en bytes de cada elemento.
			// La variable 'total_size' almacena este valor para su 
			// uso posterior en el programa.

			// 3) Calculo el tamaño total de la memoria que se va 
			// a reservar, y se reserva esa cantidad de memoria 
			// utilizando 'malloc'. Si 'malloc' devuelve 
			// NULL, la función 'ft_calloc' devuelve NULL inmediatamente, 
			// indicando que no se pudo reservar suficiente memoria.
			...
			// unsigned char utilizando un casteo. Se utiliza unsigned char 
			// en lugar de char para asegurarse de que los valores almacenados 
			// sean siempre positivos (en el rango de 0 a 255). 
